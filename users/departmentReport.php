...
// If admin, they can view all departments or select one
// If manager, they can only view their own department
if ($user_role === 'admin') {
    // Get selected department or default to first one
    $selected_dept_id = isset($_GET['dept_id']) ? $_GET['dept_id'] : null;
    
    if (!$selected_dept_id) {
        $first_dept = $pdo->query("SELECT dept_id FROM departments ORDER BY dept_name LIMIT 1")->fetch();
        $selected_dept_id = $first_dept['dept_id'] ?? null;
    }
    
    // Get all departments for dropdown
    $all_departments = $pdo->query("SELECT dept_id, dept_name, dept_code FROM departments ORDER BY dept_name")->fetchAll(PDO::FETCH_ASSOC);
} else {
    // Manager can only see their department
    $dept_query = $pdo->prepare("SELECT dept_id FROM departments WHERE manager_id = ?");
    $dept_query->execute([$manager_id]);
    $manager_dept = $dept_query->fetch();
    
    if (!$manager_dept) {
        die("You are not assigned as a manager to any department.");
    }
    
    $selected_dept_id = $manager_dept['dept_id'];
}

// Get department details
$dept_stmt = $pdo->prepare("
    SELECT d.dept_id, d.dept_name, d.dept_code, d.description, d.manager_id, 
           d.location, d.budget, d.is_active, d.created_at,
    ...
    WHERE d.dept_id = ?
");
$dept_stmt->execute([$selected_dept_id]);
...
$department['manager_name'] = trim($department['manager_name'] ?? '');
$department['manager_email'] = $department['manager_email'] ?? '';
$department['location'] = $department['location'] ?? 'Not Specified';
$department['budget'] = $department['budget'] ?? 0;
$department['description'] = $department['description'] ?? '';
$department['is_active'] = $department['is_active'] ?? 1;

// Initialize arrays and stats
$employees = [];
$category_breakdown = [];
$assignments = [];
$activities = [];
...
try {
    $emp_stats_query = $pdo->prepare("
        ...
        WHERE department = ?
    ");
    $emp_stats_query->execute([$department['dept_name']]);
...
try {
    ...
    ");
    $employees_query->execute([$department['dept_name']]);
...
$asset_stats_query = $pdo->prepare("
    ...
    WHERE department = ?
");
$asset_stats_query->execute([$department['dept_name']]);
...
$assets_by_category = $pdo->prepare("
    ...
    ORDER BY count DESC
");
$assets_by_category->execute([$department['dept_name']]);
...
$recent_assignments = $pdo->prepare("
    ...
    LIMIT 10
");
$recent_assignments->execute([$department['dept_name']]);
...
");
$asset_value_stats->execute([$department['dept_name']]);
...
$top_assets = $pdo->prepare("
    ...
    LIMIT 5
");
$top_assets->execute([$department['dept_name']]);
$top_valuable_assets = $top_assets->fetchAll(PDO::FETCH_ASSOC);

// Calculate budget utilization
$budget_utilization = 0;
if ($department['budget'] > 0) {
    $budget_utilization = round(($asset_stats['total_value'] / $department['budget']) * 100, 1);
}
...
$employee_assets = $pdo->prepare("
    ...
    FROM users u
    LEFT JOIN assets a ON u.user_id = a.assigned_to AND a.department = ?
    ...
    LIMIT 10
");
$employee_assets->execute([$department['dept_name'], $department['dept_name']]);
...
<head>
    ...
    <title>Department Report - <?php echo htmlspecialchars($department['dept_name']); ?></title>
    <style>
        * {
            margin: 0;
            padding: 0;
        ...
        @media print {
            ...
            .header-actions, .print-btn, .btn, .export-btn {
...
<body>
    ...
    <div class="container">
        <div class="header">
            ...
            <div class="header-subtitle">
                Comprehensive overview of <?php echo htmlspecialchars($department['dept_name']); ?>
            </div>
            <div class="header-actions">
                <?php if ($user_role === 'admin'): ?>
                <div class="dept-selector">
                    <label style="color: white;">Department:</label>
                    <select onchange="window.location.href='departmentReport.php?dept_id=' + this.value">
                        <?php foreach ($all_departments as $dept): ?>
                        <option value="<?php echo $dept['dept_id']; ?>" <?php echo $dept['dept_id'] == $selected_dept_id ? 'selected' : ''; ?>>
        ...
        <div class="dept-info-section">
            <div class="dept-details">
                <div class="dept-detail-item">
                    ...
                    <p><?php echo htmlspecialchars($department['dept_code']); ?></p>
                </div>
                <div class="dept-detail-item">
                    <h4>Department Manager</h4>
                    <p><?php echo htmlspecialchars($department['manager_name'] ?: 'Not Assigned'); ?></p>
                </div>
                <div class="dept-detail-item">
                    <h4>Location</h4>
                ...
                <div class="dept-detail-item">
                    <h4>Annual Budget</h4>
                    <p>$<?php echo number_format($department['budget'], 2); ?></p>
                </div>
                <div class="dept-detail-item">
                    <h4>Status</h4>
                    <p>
                        <span class="badge badge-<?php echo $department['is_active'] ? 'active' : 'inactive'; ?>">
        ...
        <div class="content-section">
            ...
            <div class="summary-grid">
                ...
                <div class="summary-card">
                    ...
                    <div class="description">
                        $<?php echo number_format($asset_stats['total_value'], 0); ?> of $<?php echo number_format($department['budget'], 0); ?> budget
                    </div>
                </div>
                <div class="summary-card">
        ...
        <div class="content-section">
            <div class="section-header">
                <h2 class="section-title">💎 Top Valuable Assets</h2>
            </div>
        ...
        <div class="content-section">
            <div class="section-header">
                <h2 class="section-title">👤 Employee Asset Distribution</h2>
            </div>
            <div class="table-container">
                <table>
                    <thead>
        ...
        <div class="content-section">
            <div class="section-header">
                <h2 class="section-title">📊 Asset Status Overview</h2>
            </div>
            <div class="stats-grid">
                <div class="stat-card">
        ...
        <div class="content-section">
            <div class="section-header">
                <h2 class="section-title">📋 Recent Asset Assignments</h2>
            </div>
        ...
        <div class="content-section">
            <div class="section-header">
                <h2 class="section-title">👥 Department Employees</h2>
                <div>
    ...
    <script>
        // Department data
        const deptData = {
            name: <?php echo json_encode($department['dept_name']); ?>,
            code: <?php echo json_encode($department['dept_code']); ?>,
            manager: <?php echo json_encode($department['manager_name'] ?: 'Not Assigned'); ?>,
            location: <?php echo json_encode($department['location']); ?>,
            budget: '<?php echo number_format($department['budget'], 2); ?>',
            status: '<?php echo $department['is_active'] ? 'Active' : 'Inactive'; ?>',
            stats: {
                totalEmployees: <?php echo $emp_stats['total_employees']; ?>,
                activeEmployees: <?php echo $emp_stats['active_employees']; ?>,
                totalAssets: <?php echo $asset_stats['total_assets']; ?>,
                availableAssets: <?php echo $asset_stats['available_assets']; ?>,
                inUseAssets: <?php echo $asset_stats['in_use_assets']; ?>,
                maintenanceAssets: <?php echo $asset_stats['maintenance_assets']; ?>,
                totalValue: '<?php echo number_format($asset_stats['total_value'], 2); ?>',
                avgValue: '<?php echo number_format($asset_stats['avg_value'], 2); ?>',
                utilizationRate: '<?php echo $utilization_rate; ?>',
                budgetUtilization: '<?php echo $budget_utilization; ?>'
            },
            employees: <?php echo json_encode(array_map(function($emp) {
                return [
        ...
        function exportToCSV() {
            ...
            csvData.push(['Department Code', deptData.code]);
            csvData.push(['Manager', deptData.manager]);
            csvData.push(['Location', deptData.location]);
            csvData.push(['Budget', '$' + deptData.budget]);
            csvData.push(['Status', deptData.status]);
            csvData.push([]);
